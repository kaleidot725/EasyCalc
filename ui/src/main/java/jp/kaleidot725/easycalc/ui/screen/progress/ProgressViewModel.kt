...
import jp.kaleidot725.easycalc.domain.model.question.Answer
...
class ProgressViewModel(
    ...
    override fun onClickNumber(number: Int) = intent {
        ...
        when (state.focusMode) {
            FocusMode.ANSWER -> {
                ...
                val isInvalidRemainder = state.question.hasRemainder && state.question.remainder != state.textRemainder
            ...
            FocusMode.REMAINDER -> {
                ...
                val isInvalidRemainder = state.question.hasRemainder && state.question.remainder != next
                if (isInvalidAnswer || isInvalidRemainder) return@intent
            }
        }

        qaList.add(state.question, Answer.Success(state.textAnswer, state.textRemainder))
    ...
    override fun onTimeout() = intent {
        if (state.isSuccess || state.isFailed) return@intent
        qaList.add(state.question, Answer.Failed(state.textAnswer, state.textRemainder))
    ...
    override fun onDelete() = intent {
        ...
        when (state.focusMode) {
            ...
            FocusMode.REMAINDER -> {
                ...
                val isInvalidAnswer = state.question.answer != state.textAnswer
                val isInvalidRemainder = state.question.hasRemainder && state.question.remainder != textRemainder
                if (isInvalidAnswer || isInvalidRemainder) return@intent
            }
        }
